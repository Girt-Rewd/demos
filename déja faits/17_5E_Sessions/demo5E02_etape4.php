<?php
	...
	// si les paramètres essentiels nom et prénom n'ont pas été initialisés, on retoune à l'étape 1 
	if (!ISSET($_SESSION['nom']) OR !ISSET($_SESSION['prenom'])) {
		header("Location: demo5E02_etape1.php");
	}

	// si le paramètre essentiel profession n'ont pas été initialisés, on retoune à l'étape 2 
	if (!ISSET($_SESSION['profession'])) {
		header("Location: demo5E02_etape2.php");
	}

	// Dans tous les autres cas, on enregistre le numéro de l'étape dans la variable etapeCV


	// On récupère les variables de session, pour les utiliser dans le HTML final
	$_SESSION['etapeCV']=4;	
